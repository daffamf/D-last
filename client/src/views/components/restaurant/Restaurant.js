...
import RestaurantItem from '../../containers/restaurant/RestaurantItem';
...
class Restaurant extends Component {
    ...
    componentDidMount() {
        this.props.loadRestaurant(this.state.kategori);
    }
    ...
    handleBtnAdd() {
        window.location = "/admin/restaurant/add"
    }

    handleBtnSearch = (event) => {
        if (this.state.search) {
            this.props.loadRestaurant(this.state.kategori, this.state.page, this.state.search)
        }else{ 
            this.props.loadRestaurant(this.state.kategori, this.state.page)
        }
    ...
    handlePageChange = (event, value) => {
        ...
        if(this.state.search === ''){
            this.props.loadRestaurant(this.state.kategori, value);
        }else{
            this.props.loadRestaurant(this.state.kategori, value, this.state.search);
        }
        event.preventDefault();
    }

    render() {
        const restaurantNode = this.props.data.data.map((item, index) =>
            <RestaurantItem
                key={index}
                id={item.id}
                            ...
                            <div className="col-9">
                                <div className="card bg-transparent">
                                    <div className="contact-header bg-white">
                                        <h4 className="font-20">Restaurants</h4>
                                    </div>
...
const mapDispatchToProps = (dispatch) => ({
    loadRestaurant: (kategori,current_page, search) => dispatch(loadRestaurant(kategori,current_page, search))
})
